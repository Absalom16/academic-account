...
try {
 ...
 $q = "SELECT COUNT(id) FROM filedownload";
 ...
 $query = "SELECT filename, "; // #5
 $query .= "DATE_FORMAT(upload_date, '%M %d, %Y')";
 $query .=
 " AS regdat, id FROM filedownload ORDER BY upload_date ASC";
 $query .=" LIMIT ?, ?";
...
$upload_date = htmlspecialchars($row['regdat'], ENT_QUOTES);
 echo '<tr>
 ...
 $echostring = "<p class='text-center'>Total files: $members </p>";
 ...
 if ($current_page != 1) {
 $echostring .=
 '<a href="view_files.php?s=' . ($start - $pagerows) .
 '&p=' . $pages . '">Previous</a> ';
 }
